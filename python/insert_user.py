...
from models import User, LossReason, FoodLossRecord  # 必要なモデルをインポート
import hashlib
import logging
...
def add_test_data():
    ...
    try:
        ...
        if not test_user:
            ...
            test_user = User(
                username="test_user", password=hashed_password, email="test@example.com"
            )
        ...
        expired_reason = (
            session.query(LossReason).filter_by(reason_text="期限切れ").first()
        )
        eaten_reason = (
            session.query(LossReason).filter_by(reason_text="食べ残し").first()
        )

        if not expired_reason or not eaten_reason:
            logger.error(
                "Error: Loss reasons not found. Please run database initialization."
            )
            return
